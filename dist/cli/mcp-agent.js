...
import { getAgentTools, handleAgentTool, } from './managers/mcp-tools-manager/index.js';
...
function createConnectionHandler(mode) {
    return async (socket) => {
        ...
        clientServer.setRequestHandler(ListToolsRequestSchema, () => {
            return { tools: TOOLS };
        });
        clientServer.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (result.isError)
                log('WARN', `Error: ${name}`, { connId, taskId: clientTaskId });
            return result;
        });
