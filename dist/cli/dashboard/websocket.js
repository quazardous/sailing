 ...
 */
function handleFrame(buffer, connection) {
    if (buffer.length < 2)
        return;
 ...
 * Send agent log line
 */
export function sendAgentLog(taskId, line) {
    broadcast({
        type: 'agent:log',
        taskId,
        line,
 ...
 */
export function sendAgentStatus(taskId, status) {
    broadcast({
        type: 'agent:status',
        taskId,
        status,
 ...
 */
export function sendArtefactUpdated(id) {
    broadcast({
