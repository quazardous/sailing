...
import { setCacheTTL, clearCache, getCachedPrdsData, getCachedBlockers, getCachedPendingMemory, getMemoryContent, loadView, render, getStatusBadge, markdownToHtml, entityLink, renderMeta, renderTabs, getProgressClass, getKpiClass, generatePrdDag, generateEpicDag, generateTaskDag, renderDag, renderPrdDag, generatePrdGantt, generateEpicGantt, generatePrdOverviewGantt, renderGantt, renderSimpleGantt, 
// Templates
welcomeTemplate, versionCardTemplate, prdCardTemplate, epicCardTemplate, taskItemTemplate, detailHeaderTemplate, progressBarTemplate, kpiGridTemplate, taskStatsTemplate, versionDetailTemplate, blockerItemTemplate, memoryWarningTemplate, versionItemTemplate, settingsTemplate, pathRowTemplate, configRowTemplate, emptyTemplate } from './lib/index.js';
// Routes
export function createRoutes(options = {}) {
    ...
    `;
        ...
        const ganttContent = renderGantt(ganttData.tasks, ganttData.criticalPath, ganttData.title, ganttData.totalHours, ganttData.t0, ganttData.durationHours, ganttData.criticalTimespanHours);
        html(res, `
    ...
    routes['/api/task/:id'] = (req, res) => {
        ...
        const taskMeta = foundTask.meta;
        const statsContent = taskStatsTemplate({
            status: foundTask.status,
            effort: taskMeta?.effort,
            startedAt: taskMeta?.started_at,
            doneAt: taskMeta?.done_at,
