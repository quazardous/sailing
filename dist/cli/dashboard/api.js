...
function toApiSimpleGantt(result) {
    return {
        tasks: result.tasks.map(t => ({
            id: t.id,
            name: t.name,
 ...
 */
export function createApiV2Routes() {
    const routes = {};
            ...
            if (id.startsWith('T')) {
                for (const prd of prds) {
                    for (const epic of prd.epics) {
                        ...
                        if (task) {
                            const dagData = generateStructuredTaskDag(task, { id: epic.id, title: epic.title, status: epic.status }, { id: prd.id, title: prd.title, status: prd.status });
                            const response = {
                                type: 'task',
                                data: task,
                                dag: dagData,
    ...
    routes['/api/v2/agents'] = (_req, res) => {
        try {
            ...
            const agentInfos = Object.entries(agentsDb).map(([taskId, agent]) => ({
                taskId,
            ...
            const response = { agents: agentInfos };
            json(res, response);
        }
        catch (error) {
            console.error('API error:', error);
