...
import { parseMcpCliArgs, setProjectRoot, setLogFile, log, SocketTransport, createSocketServer, createTcpServer, } from '../managers/mcp-manager.js';
import { getConductorTools, handleConductorTool, } from '../managers/mcp-tools-manager/index.js';
...
function createConnectionHandler(mode) {
    return async (socket) => {
        ...
        const clientServer = new Server({ name: 'rudder-mcp-conductor', version: '1.0.0' }, { capabilities: { tools: {} } });
...
async function main() {
    ...
    if (port) {
        ...
        // Stdio mode
        const server = new Server({ name: 'rudder-mcp-conductor', version: '1.0.0' }, { capabilities: { tools: {} } });
