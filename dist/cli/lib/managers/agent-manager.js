...
import { loadFile, saveFile, findProjectRoot } from '../../managers/core-manager.js';
import { loadState, saveState } from '../state.js';
import { getGit } from '../git.js';
import { getAgentConfig } from '../../managers/core-manager.js';
import { removeWorktree } from '../worktree.js';
import { getTask } from '../artefacts.js';
import { normalizeId } from '../normalize.js';
import { parseUpdateOptions } from '../update.js';
import { getAgentDir, checkAgentCompletion } from '../agent-utils.js';
// ============================================================================
// Wait / Monitor
// ============================================================================
/**
 ...
 */
export function isAgentRunning(taskId) {
 ...
 */
export async function waitForAgent(taskId, timeoutSec) {
 ...
 */
export async function checkMergeConflicts(taskId) {
    taskId = normalizeId(taskId);
    const state = loadState();
    const agentInfo = state.agents?.[taskId];
 ...
 */
export async function autoCommitChanges(taskId) {
    taskId = normalizeId(taskId);
    const state = loadState();
    const agentInfo = state.agents?.[taskId];
 ...
 */
export async function mergeAgentWork(taskId, strategy = 'merge') {
    taskId = normalizeId(taskId);
    const state = loadState();
    const agentInfo = state.agents?.[taskId];
    if (!agentInfo?.worktree) {
 ...
 */
export async function reapAgent(taskId, options = {}) {
    taskId = normalizeId(taskId);
    ...
    if (agentInfo.worktree) {
        ...
        const mergeResult = await mergeAgentWork(taskId, strategy);
 ...
 * Kill agent process
 */
export async function killAgent(taskId) {
    taskId = normalizeId(taskId);
    const state = loadState();
    const agentInfo = state.agents?.[taskId];
    if (!agentInfo) {
 ...
 */
export async function rejectAgent(taskId, reason) {
    taskId = normalizeId(taskId);
    const state = loadState();
    const agentInfo = state.agents?.[taskId];
    if (!agentInfo) {
 ...
 */
export function clearAgent(taskId) {
    taskId = normalizeId(taskId);
    const state = loadState();
