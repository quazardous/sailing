...
export function composeContext(options) {
    const { operation, role = null, debug = false, includeHeader = true, includeWorkflow = true, includeProjectFiles = true } = options;
 ...
 */
function buildAgentSpawnPromptLegacy(taskId, options = {}) {
    const agentConfig = getAgentConfig();
    const useWorktree = options.useWorktree ?? agentConfig.use_worktrees ?? true;
    const taskEntry = getTask(taskId);
    if (!taskEntry)
