 ...
 *
 * Pure technical operations for GitHub and GitLab PR/MR management.
 * Config-dependent logic is in managers/pr-manager.ts
 */
...
// Note: getProvider() moved to managers/pr-manager.ts (needs config access)
/**
 ...
 * @param {string} cwd - Working directory
 * @param {'github'|'gitlab'} provider - Provider type (auto-detected if not provided)
 * @returns {object|null} PR status or null
 */
export async function getStatus(branch, cwd, provider) {
    provider = provider || await detectProvider(cwd) || undefined;
    if (!provider)
        return null;
    try {
        if (provider === 'github') {
