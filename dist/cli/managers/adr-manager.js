 ...
 */
export function getNextAdrNumber() {
 ...
 * Create a new ADR
 */
export function createAdr(title, options = {}) {
 ...
 * Update ADR status
 */
export function updateAdrStatus(id, newStatus, options = {}) {
    const entry = getAdr(id);
    if (!entry)
        return false;
    const loaded = loadFile(entry.file);
    ...
    if (options.supersededBy && newStatus === 'Superseded') {
        ...
        if (supersedesEntry) {
            ...
            if (supersedesLoaded) {
                const supersedesData = supersedesLoaded.data;
 ...
 * Get ADRs by status
 */
export function getAdrsByStatus(status) {
    return getAllAdrs().filter(e => e.data.status === status);
}
/**
 * Get ADRs by domain
 */
export function getAdrsByDomain(domain) {
    return getAllAdrs().filter(e => e.data.domain === domain);
}
/**
 * Get ADRs by tags
 */
export function getAdrsByTags(tags) {
 ...
 */
export function getAcceptedAdrs() {
 ...
 */
export function normalizeAdrId(input) {
 ...
 */
export function formatAdrLine(entry) {
 ...
 */
export function formatAdrsForPrompt(adrs) {
    if (adrs.length === 0)
        return '';
