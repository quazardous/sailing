...
function getScriptInfo(basePath) {
...
export class ServiceManager {
    ...
    async startMcp(options = {}) {
        ...
        const getTransportArgs = (service, offset) => {
            if (mcpMode === 'port') {
    ...
    stopMcp() {
        ...
        try {
            ...
            if (state.conductor?.pid)
                pidsToKill.push(state.conductor.pid);
            if (state.agent?.pid)
                pidsToKill.push(state.agent.pid);
            // Kill processes
