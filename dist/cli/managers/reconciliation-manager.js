 ...
 */
export function getBranchState(branch, parent) {
...
export function report(context = {}) {
    ...
    lines.push('');
    // Hierarchy
    const branchHierarchy = diag.hierarchy.filter((h) => typeof h === 'object' && h !== null && !!h.branch);
    if (branchHierarchy.length > 0) {
        ...
        for (const h of branchHierarchy) {
            ...
            if (h.ahead)
                lines.push(`      ${h.ahead} ahead`);
            if (h.behind)
                lines.push(`      ${h.behind} behind`);
        }
        lines.push('');
    }
    // Worktrees
