...
import { ok, err, normalizeId, detectType, canonicalId } from '../types.js';
                    ...
                    if (scopeType === 'prd') {
                        ...
                        if (prdMatch) {
                            ...
                            items = prdTasks
                                ...
                                .filter(t => !tags?.length || tags.some((tag) => (t.data?.tags || []).includes(tag)))
                                .map(t => ({
                                id: canonicalId(t.id),
                                title: t.data?.title,
    ...
    },
    {
        ...
        handler: (args) => {
            ...
            try {
                ...
                const warnings = [];
                if (type === 'epic') {
                    const epicStatus = file.data?.status;
