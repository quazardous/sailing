...
test('assertValidHtmlComments: throws on unclosed comment', () => {
    ...
    try {
        ...
        threw = true;
        assert(e.message.includes('Unclosed HTML comment'), 'Error should mention unclosed comment');
        assert(e.message.includes('test'), 'Error should include context');
    }
...
test('saveFile throws on unclosed comment in body', () => {
    ...
    try {
        ...
        threw = true;
        assert(e.message.includes('Unclosed HTML comment'), 'Should mention unclosed comment');
    }
    assert(threw, 'Should have thrown');
