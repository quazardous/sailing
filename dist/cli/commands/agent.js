...
import { getTask, getEpic, getTaskEpic, getMemoryFile } from '../lib/index.js';
import { normalizeId } from '../lib/normalize.js';
import { getAgentsBaseDir, getAgentDir, getProcessStats, formatDuration, checkAgentCompletion } from '../lib/agent-utils.js';
...
export function registerAgentCommands(program) {
    ...
    withModifies(agent.command('reap <task-id>'), ['task', 'git', 'state'])
        ...
        if (agentInfo.worktree) {
            ...
            // 5d. Cleanup worktree (only if requested)
            if (options.cleanupWorktreeAfter) {
                ...
                if (!options.json) {
                    console.log(`✓ Merged ${taskId} (worktree kept for debug)`);
                }
            }
        }
    ...
    withModifies(agent.command('check'), ['task'])
        ...
        try {
            ...
            if (mcpStatus.mode === 'port') {
                ...
                connectResult = execSync(`echo '${testRequest}' | timeout 5 nc 127.0.0.1 ${mcpStatus.port}`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
            }
            else {
                debug(`Testing: echo | socat - UNIX-CONNECT:${mcpStatus.socket}`);
                connectResult = execSync(`echo '${testRequest}' | timeout 5 socat - UNIX-CONNECT:${mcpStatus.socket}`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
            }
        ...
        let testSocket = mcpStatus.socket;
        ...
        try {
            ...
            const sandboxTestResult = execSync(`timeout 10 srt --settings ${srtConfigForTest} sh -c "${sandboxTestCmd}"`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
        ...
        let spawnBridgeCleanup = null;
        let spawnSocket = mcpStatus.socket;
        if (mcpStatus.mode === 'port' && isLinux) {
