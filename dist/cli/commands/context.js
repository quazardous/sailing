...
function composeContext(operation, options = {}) {
    ...
    function processInjectDimension(dimension) {
        ...
        if (Array.isArray(dimension)) {
            ...
            if (dimension.files)
                projectFiles.push(...dimension.files);
            if (dimension.fragments)
                additionalFragments.push(...dimension.fragments);
            if (dimension.exclude)
                dimension.exclude.forEach(f => excludeFragments.add(f));
        }
    }
    ...
    for (const fragmentPath of additionalFragments) {
        if (excludeFragments.has(fragmentPath))
            continue;
        const content = loadFragment(fragmentPath);
        if (content) {
            parts.push(content);
