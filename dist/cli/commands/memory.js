...
export function registerMemoryCommands(program) {
    ...
    memory.command('show <id>')
        ...
        if (allSections.length === 0) {
            ...
            if (!options.full)
                console.log('  (use --full to show all sections)');
            return;
        }
    ...
    memory.command('sync')
        ...
        for (const { id: taskId, path: taskPath } of taskLogs) {
            ...
            if (targetEpicId) {
                ...
                if (!taskInfo || taskInfo.epicId !== targetEpicId)
                    continue;
            }
        ...
        if (archivedOrphans > 0)
            console.log(`Archived: ${archivedOrphans} orphan log(s) → archive/memory/`);
        if (mergedTasks > 0)
            console.log(`Merged: ${mergedTasks} task logs → epic logs`);
        if (deletedEmpty > 0)
            console.log(`Deleted: ${deletedEmpty} empty logs`);
        if (createdMd > 0)
            console.log(`Created: ${createdMd} memory files`);
