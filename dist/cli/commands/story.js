...
function findStoryFile(storyId) {
    ...
    for (const prdDir of findPrdDirs()) {
        ...
        for (const f of files) {
            if (matchesId(f, storyId))
                return { file: f, prdDir };
        }
    }
    return null;
}
 ...
 */
function getAllStories(prdFilter = null, includePath = false) {
    const stories = [];
    for (const prdDir of findPrdDirs()) {
        if (prdFilter && !matchesPrdDir(prdDir, prdFilter))
            continue;
...
export function registerStoryCommands(program) {
    ...
    story.command('create <prd> <title>')
        ...
        if (options.json) {
            const output = { id, title, parent: data.parent, type: data.type };
            if (options.path)
