...
import { loadNoiseFilters, saveNoiseFilters, matchesNoiseFilter, parseJsonLog, analyzeLog, printDiagnoseResult } from '../lib/diagnose.js';
/**
 * Summarize an event for display
 */
function summarizeEvent(event, line) {
 ...
 */
function resolveEpicId(taskOrEpic) {
...
export function registerDiagnoseCommands(program) {
    ...
    diagnose.command('add-filter <id> [task-or-epic]')
        ...
        if (options.type)
            newFilter.match.type = options.type;
        if (options.contains)
            newFilter.match.contains = options.contains;
        if (options.pattern)
        ...
        if (options.json) {
            ...
            if (filters.length === 0) {
                ...
                for (const f of filters) {
                    ...
                    if (f.match.type)
                        console.log(`    type: ${f.match.type}`);
                    if (f.match.contains)
                        console.log(`    contains: ${f.match.contains}`);
                    if (f.match.pattern)
                        console.log(`    pattern: ${f.match.pattern}`);
                }
            }
        }
    });
