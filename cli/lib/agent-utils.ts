...
export interface AgentUtilsOptions {
  json?: boolean;
  escalate?: (reason: string, steps?: string[]) => void;
}
...
export interface AgentsMap {
  [taskId: string]: AgentCompletionInfo & Record<string, unknown>;
}
...
export class AgentUtils {
   ...
   */
  getAgentDir(taskId: string): string {
   ...
   */
  checkCompletion(taskId: string, agentInfo?: AgentCompletionInfo): CompletionResult {
   ...
   */
  dirExists(taskId: string): boolean {
 ...
 */
export function getAgentOrEscalate(
  taskId: string,
  agents: AgentsMap,
  options: AgentUtilsOptions = {}
): { taskId: string; agent: AgentsMap[string] } | null {
 ...
 */
export function getProcessStats(pid: number): { running: boolean; cpu?: string; mem?: string; rss?: number } {
  try {
    process.kill(pid, 0);
