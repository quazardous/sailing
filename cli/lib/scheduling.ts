...
/**
 * Task data required for scheduling
 */
...
/**
 * Task data with real dates for realistic scheduling
 */
export interface RealSchedulableTask extends SchedulableTask {
  status: string;
...
/**
 * Effort configuration from project config
 */
...
/**
 * Schedule envelope for a group of tasks (PRD or Epic)
 */
export interface ScheduleEnvelope {
  totalHours: number;        // Sum of all task durations
  weightedHours: number;     // Total span considering dependencies (latest end - earliest start)
  earliestStart: number;     // Earliest start hour among all tasks
  latestEnd: number;         // Latest end hour among all tasks
  criticalPathLength: number; // Length of critical path in hours
 ...
 */
export function calculateTheoreticalSchedule(
  taskData: Map<string, SchedulableTask>,
  ...
  for (const id of taskIds) {
    if (!schedule.has(id)) {
      ...
      for (const s of schedule.values()) {
        if (s.endHour > maxEnd) maxEnd = s.endHour;
      }
 ...
 * @returns Map of task ID to schedule (startHour, endHour, durationHours)
 */
export function calculateRealSchedule(
  ...
  const dateToHours = (isoDate: string): number => {
  ...
  const getMaxBlockerEnd = (blockedBy: string[]): number => {
    let maxEnd = 0;
    for (const blockerId of blockedBy) {
      if (!taskData.has(blockerId)) continue;
...
export function calculateCriticalPath(
  taskData: Map<string, { id: string; blockedBy: string[] }>,
  ...
  let projectEnd = 0;
  for (const s of schedule.values()) {
    if (s.endHour > projectEnd) projectEnd = s.endHour;
  }
  ...
  while (toProcess.length > 0) {
    ...
    for (const depId of task.blockedBy) {
      if (criticalPath.has(depId)) continue;
 ...
 */
export function getScheduleEnvelope(schedule: Map<string, TaskSchedule>): ScheduleEnvelope {
  if (schedule.size === 0) {
    return {
      totalHours: 0,
      weightedHours: 0,
  ...
  for (const s of schedule.values()) {
    ...
    if (s.startHour < earliestStart) earliestStart = s.startHour;
    if (s.endHour > latestEnd) latestEnd = s.endHour;
  }
...
export function delayTask(
  ...
  taskId: string,
  delayHours: number,
  taskData: Map<string, { blockedBy: string[] }>
): Map<string, TaskSchedule> {
  if (delayHours <= 0) return schedule;
 ...
 */
export function calculateParallelEfficiency(envelope: ScheduleEnvelope): number {
  if (envelope.weightedHours === 0) return 1;
...
export interface GanttMetrics {
  /** Real span: maxEndHour - minStartHour */
  realSpanHours: number;
  /** Display span: for chart width, excludes in-progress "now" extension */
  displaySpanHours: number;
  /** Total effort: sum of all task durations */
  totalEffortHours: number;
  /** Critical timespan: theoretical minimum span (from critical path) */
  criticalTimespanHours: number;
  /** Earliest start hour among all tasks */
  minStartHour: number;
  /** Latest end hour among all tasks */
  maxEndHour: number;
  /** Display max end hour: for chart width */
  displayMaxEndHour: number;
 ...
 */
export function calculateGanttMetrics(
  taskData: Map<string, RealSchedulableTask>,
  effortConfig: EffortConfig,
  t0: Date,
  now?: Date
  ...
  for (const s of realSchedule.values()) {
    if (s.endHour > maxEndHour) maxEndHour = s.endHour;
    if (s.startHour < minStartHour) minStartHour = s.startHour;
  }
  if (minStartHour === Infinity) minStartHour = 0;
  ...
  for (const [taskId, s] of realSchedule) {
    ...
    if (displayEnd > displayMaxEndHour) displayMaxEndHour = displayEnd;
  }
