 ...
 */
export function registerStatusCommands(worktree: Command): void {
  // worktree:status
    ...
    .action(async (options: StatusOptions) => {
      ...
      for (const [taskId, info] of Object.entries(agents)) {
        ...
        const entry = {
          ...
          pr: null as { url: string; state?: string; mergeable?: boolean } | null
        };
  ...
  worktree.command('preflight')
    ...
    .action(async (options: PreflightOptions) => {
      ...
      const pendingMerges: Array<{ taskId: string; pr_url: string | null; commits: number }> = [];
