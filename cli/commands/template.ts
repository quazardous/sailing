 ...
 */
function collectVar(val: string, acc: Record<string, string>) {
 ...
 */
function getConfigDefaults(): Record<string, string> {
  try {
...
export function registerTemplateCommands(program: Command) {
  ...
  tpl.command('render')
    ...
    .action((templatePath: string, options: { var: Record<string, string>; output?: string }) => {
