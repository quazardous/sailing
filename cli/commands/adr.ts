...
import {
  ...
  normalizeAdrId,
  getAdrDir
} from '../managers/adr-manager.js';
import { addDynamicHelp } from '../lib/help.js';
import type { Command } from 'commander';
import type { AdrStatus } from '../lib/types/adr.js';
...
export function registerAdrCommands(program: Command): void {
  ...
  adr.command('list')
    ...
    .action((options: ListOptions) => {
      ...
      // Apply filters
      if (options.status) {
        const status = options.status as AdrStatus;
  ...
  adr.command('show <id>')
    ...
    .action((id: string, options: ShowOptions) => {
      ...
      if (options.json) {
        ...
        if (fullAdr.author) console.log(`Author:  ${fullAdr.author}`);
        if (fullAdr.domain) console.log(`Domain:  ${fullAdr.domain}`);
        if (fullAdr.tags && fullAdr.tags.length > 0) {
          console.log(`Tags:    ${fullAdr.tags.join(', ')}`);
        }
        if (fullAdr.supersedes) console.log(`Supersedes: ${fullAdr.supersedes}`);
        if (fullAdr.superseded_by) console.log(`Superseded by: ${fullAdr.superseded_by}`);
  ...
  adr.command('accept <id>')
    ...
    .option('--json', 'JSON output')
    .action((id: string, options: StatusChangeOptions) => {
      const normalizedId = normalizeAdrId(id);
  ...
  adr.command('deprecate <id>')
    ...
    .action((id: string, options: StatusChangeOptions) => {
      const normalizedId = normalizeAdrId(id);
      const newStatus: AdrStatus = options.supersededBy ? 'Superseded' : 'Deprecated';
