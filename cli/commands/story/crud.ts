...
import type { StoryListOptions, StoryShowOptions, StoryCreateOptions, StoryUpdateOptions } from './helpers.js';

/**
 * Register story CRUD commands
 */
export function registerCrudCommands(story: Command): void {
  // story:list
  ...
  story.command('show <id>')
    ...
    .action((id: string, options: StoryShowOptions) => {
      ...
      const childrenList: StoryWithPrd[] = children.get(storyId) || [];
  ...
  story.command('create <prd> <title>')
    ...
    .action((prd: string, title: string, options: StoryCreateOptions) => {
      ...
      if (options.json) {
        const output: Record<string, unknown> = { id: storyId, title, parent: data.parent, type: data.type };
  ...
  story.command('update <id>')
    ...
    .option('--set <key=value>', 'Set any frontmatter field (repeatable)', (v, arr: string[]) => arr.concat(v), [] as string[])
    .option('--json', 'JSON output')
