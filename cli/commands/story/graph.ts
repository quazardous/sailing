/**
 * Story graph commands (tree, roots, leaves, children, ancestors)
 */
...
import type { StoryTreeOptions, StoryRootsOptions, StoryLeavesOptions, StoryChildrenOptions, StoryAncestorsOptions } from './helpers.js';

/**
 * Register story graph commands
 */
export function registerGraphCommands(story: Command): void {
  // story:tree
  story.command('tree [prd]')
    ...
    .action((prdArg: string | undefined, options: StoryTreeOptions) => {
      ...
      function printTree(story: StoryWithPrd, indent = '') {
        const typeIcon = story.type === 'user' ? '👤' : story.type === 'technical' ? '⚙️' : '🔌';
        console.log(`${indent}${typeIcon} ${story.id}: ${story.title}`);
        const childList: StoryWithPrd[] = children.get(normalizeId(story.id)) || [];
  ...
  story.command('leaves [prd]')
    ...
    .action((prdArg: string | undefined, options: StoryLeavesOptions) => {
      ...
      const leaves = stories.filter(s => {
        const childList: StoryWithPrd[] = children.get(normalizeId(s.id)) || [];
  ...
  story.command('children <id>')
    ...
    .action((id: string, options: StoryChildrenOptions) => {
      ...
      const childList: StoryWithPrd[] = children.get(storyId) || [];
  ...
  story.command('ancestors <id>')
    ...
    .action((id: string, options: StoryAncestorsOptions) => {
      ...
      const ancestors: StoryWithPrd[] = [];
      let current: StoryWithPrd | undefined = byId.get(storyId);
