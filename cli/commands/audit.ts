...
interface AuditIssue {
  type: 'epic-should-be-ready' | 'epic-should-be-in-progress' | 'prd-should-be-ready' | 'prd-should-be-in-progress' | 'epic-reopened' | 'prd-reopened';
  entity: 'epic' | 'prd';
  id: string;
  prd: string;
 ...
 */
function buildProjectStructure(): PrdInfo[] {
 ...
 */
export function auditStatusConsistency(): AuditIssue[] {
  ...
  for (const prd of prds) {
    ...
    for (const epic of prd.epics) {
      if (epic.tasks.length === 0) continue; // Skip epics with no tasks
    ...
    if (prd.epics.length === 0) continue;
 ...
 */
function applyFixes(issues: AuditIssue[], mode: 'optimistic' | 'pessimistic'): number {
  let fixedCount = 0;
...
export function registerAuditCommands(program: Command) {
  ...
  audit.command('status')
    ...
    .action((options: any) => {
      ...
      const optimisticIssues = issues.filter(i =>
        i.type === 'epic-should-be-ready' ||
        i.type === 'prd-should-be-ready' ||
        i.type === 'epic-should-be-in-progress' ||
        i.type === 'prd-should-be-in-progress'
      );
      const pessimisticIssues = issues.filter(i =>
        i.type === 'epic-reopened' ||
        i.type === 'prd-reopened'
      );
  ...
  audit.command('summary')
    ...
    .action((options: any) => {
      ...
      const summary = prds.map(prd => {
        ...
        for (const epic of prd.epics) {
          if (isStatusDone(epic.status)) epicStats.done++;
          else if (isStatusAutoDone(epic.status)) epicStats.autoDone++;
          else if (isStatusInProgress(epic.status)) epicStats.inProgress++;
          else if (isStatusNotStarted(epic.status)) epicStats.notStarted++;

          for (const task of epic.tasks) {
            taskStats.total++;
            if (isStatusDone(task.status)) taskStats.done++;
            else if (isStatusInProgress(task.status)) taskStats.inProgress++;
            else if (isStatusNotStarted(task.status)) taskStats.notStarted++;
          }
        }
