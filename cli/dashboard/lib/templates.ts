 ...
 *
 * Template functions that generate HTML fragments.
 * Each function takes typed data and returns an HTML string.
 */
import type { PrdData, BlockerData } from './types.js';
...
export function versionCardTemplate(name: string, version: string, isMain: boolean): string {
...
export function treeNodeTemplate(
  type: 'prd' | 'epic' | 'task',
  id: string,
  title: string,
  status: string,
...
export function prdCardTemplate(prd: PrdData): string {
  return `
...
export function kpiGridTemplate(kpis: KpiData[]): string {
  return `
    <div class="kpi-grid">
      ${kpis.map(kpi => `
        <div class="kpi-card">
...
export function epicCardTemplate(epic: EpicCardData): string {
...
export function taskItemTemplate(task: { id: string; title: string; status: string }): string {
...
export function detailHeaderTemplate(data: DetailHeaderData): string {
...
export function progressBarTemplate(progress: number, progressClass: string, size: 'sm' | 'lg' = 'lg'): string {
  return `
...
export function taskStatsTemplate(data: TaskStatsData, formatDate: (iso?: string) => string): string {
  const rows = [
...
export function versionDetailTemplate(name: string, version: string, isMain: boolean): string {
  return `
    <div class="detail-header">
      <h2>${name}</h2>
...
export function blockerItemTemplate(blocker: BlockerData): string {
  return `
...
export function memoryWarningTemplate(pending: string[]): string {
  if (pending.length === 0) {
...
export function versionItemTemplate(name: string, version: string, isMain: boolean): string {
...
export function settingsTemplate(data: SettingsData): string {
  return `
    <div class="detail-header">
      <h2>Settings</h2>
    </div>
...
export function pathRowTemplate(key: string, path: string | null, configured?: string, isCustom?: boolean): string {
...
export function configRowTemplate(key: string, value: unknown, description: string, defaultValue: unknown, isDefault: boolean): string {
...
export function emptyTemplate(message: string, success = false): string {
