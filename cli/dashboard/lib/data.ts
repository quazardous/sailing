...
import type { PrdData, BlockerData, EpicData, TaskData } from './types.js';

/**
 * Get all PRDs with their epics and tasks using indexes
 */
export function getPrdsDataImpl(): PrdData[] {
 ...
 */
export function getBlockersImpl(): BlockerData[] {
 ...
 */
export function getPendingMemoryImpl(): string[] {
 ...
 */
export function getMemoryContent(entityId: string, type: 'prd' | 'epic'): string {
  try {
    const memoryDir = getMemoryDir();
