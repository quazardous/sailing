 ...
 */
export async function detectProvider(cwd: string): Promise<'github' | 'gitlab' | null> {
  try {
 ...
 */
export function checkCli(provider: string): { available: boolean; cmd: string } {
 ...
 */
export async function exists(branch: string, cwd: string): Promise<boolean> {
 ...
 */
async function createCore(taskId: string, options: PrOptions = {}) {
