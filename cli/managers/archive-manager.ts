 ...
 */
async function moveFile(src: string, dest: string): Promise<'git' | 'fs'> {
 ...
 */
export function isPrdDone(prdDir: string): boolean {
 ...
 * Get PRD status
 */
export function getPrdStatus(prdDir: string): string {
  const prdFile = path.join(prdDir, 'prd.md');
 ...
 */
function getPrdMemoryFiles(targetPrdId: string): Array<{ id: string; type: string; file: string }> {
  const memoryFiles: Array<{ id: string; type: string; file: string }> = [];
 ...
 */
function addArchivedAt(prdFile: string): boolean {
  const loaded = loadFile(prdFile);
 ...
 */
export async function archivePrd(prdId: string, options: ArchiveOptions = {}): Promise<ArchiveResult> {
