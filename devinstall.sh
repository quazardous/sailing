...
read -r -d '' RUDDER_CONTENT << 'WRAPPER'
#!/bin/bash
...
if [ "$MODE" != "dev" ] && [ -f "$DIST_ENTRY" ]; then
  SAILING_PROJECT="$PROJECT_ROOT" exec node "$DIST_ENTRY" "$@"
fi

# Dev fallback: tsx if available
if command -v tsx >/dev/null 2>&1; then
  SAILING_PROJECT="$PROJECT_ROOT" exec tsx "$TS_SRC" "$@"
fi
...
read -r -d '' RUDDER_MCP_CONTENT << EOF
#!/bin/bash
