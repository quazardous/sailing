...
read -r -d '' RUDDER_CONTENT << EOF || true
#!/bin/bash
...
if [ -f "\$MODE_FILE" ] && [ "\$(cat "\$MODE_FILE" | tr -d '[:space:]')" = "dev" ]; then
  SAILING_PROJECT="\$PROJECT_ROOT" exec npx tsx "$SCRIPT_DIR/cli/rudder.ts" "\$@"
else
  SAILING_PROJECT="\$PROJECT_ROOT" exec node "$SCRIPT_DIR/dist/cli/rudder.js" "\$@"
fi
EOF

read -r -d '' RUDDER_MCP_CONTENT << EOF || true
#!/bin/bash
# Rudder MCP Server wrapper - dev mode pointing to sailing repo
# Auto-generated by devinstall.sh
export SAILING_PROJECT="\$(realpath "\$(dirname "\$0")/..")"
SAILING_REPO_ROOT="$SCRIPT_DIR"
TS_SRC="\$SAILING_REPO_ROOT/cli/mcp-server.ts"
DIST_ENTRY="\$SAILING_REPO_ROOT/dist/cli/mcp-server.js"
...
echo "rudder build not found. Run: npm run build (in \$SAILING_REPO_ROOT)"
exit 1
EOF

if [ "$DRY_RUN" = true ]; then
  echo "  Would write: bin/rudder"
  echo "  Would write: bin/rudder-mcp"
else
  echo "$RUDDER_CONTENT" > bin/rudder
  chmod +x bin/rudder
  echo -e "  ${GREEN}Created: bin/rudder${NC}"

  echo "$RUDDER_MCP_CONTENT" > bin/rudder-mcp
  chmod +x bin/rudder-mcp
  echo -e "  ${GREEN}Created: bin/rudder-mcp${NC}"
fi
echo
