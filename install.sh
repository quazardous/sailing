...
# Create bin/rudder-mcp wrapper at project root
if [ "$DRY_RUN" = true ]; then
  echo "  Would create: bin/rudder-mcp"
else
  cat > "bin/rudder-mcp" << 'WRAPPER'
#!/bin/bash
# Rudder MCP Server wrapper - points to .sailing/rudder/mcp-server.js
# This file is auto-generated by sailing installer
SCRIPT_DIR="$(dirname "$(realpath "$0")")"
exec node "$SCRIPT_DIR/../.sailing/rudder/mcp-server.js" "$@"
WRAPPER
  chmod +x "bin/rudder-mcp"
  echo -e "  ${GREEN}Installed: bin/rudder-mcp${NC}"
fi
